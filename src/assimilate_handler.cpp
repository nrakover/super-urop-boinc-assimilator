...
//    if (retval) return retval;
    ...
    if (wu.canonical_resultid) {
        ...
        for (i=0; i<n; i++) {
            ...
            if (i==0) {
                copy_path = config.project_path("%s/trans_%s", RESULTS_DIR, wu.name);
            } else if (i == 1) {
                copy_path = config.project_path("%s/emi_%s", RESULTS_DIR, wu.name);
            }
        ...
        if (!file_copied) {
            copy_path = config.project_path(
                "%s/%s_%s", RESULTS_DIR, wu.name, "no_output_files"
            );
